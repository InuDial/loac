...
fn expand_actor(implementation: &ItemImpl, args: ActorArgs) -> syn::Result<TokenStream2> {
    ...
    if let (None, Some((name, _))) = (&args.mailbox, &args.mailbox_capacity) {
        return Err(syn::Error::new(
            name.span(),
            "`mailbox_capacity` requires `mailbox`",
        ));
    }
    if let (None, Some((name, _))) = (&args.mailbox, &args.max_in_flight) {
        return Err(syn::Error::new(
            name.span(),
            "`max_in_flight` requires `mailbox`",
        ));
    }
    if let (None, Some((name, _))) = (&args.mailbox, &args.mailbox_dispatch_budget) {
        return Err(syn::Error::new(
            name.span(),
            "`mailbox_dispatch_budget` requires `mailbox`",
        ));
    }
    if let (None, Some((name, _))) = (&args.children, &args.max_children) {
        return Err(syn::Error::new(
            name.span(),
            "`max_children` requires `children`",
        ));
    }

    let mailbox_dispatch_budget = args.mailbox_dispatch_budget.as_ref().map(|(_, budget)| {
        quote! {
            const MAILBOX_DISPATCH_BUDGET: ::core::num::NonZeroUsize =
                ::core::num::NonZeroUsize::new(#budget)
                    .expect("mailbox dispatch budget must be greater than zero");
        }
    });

    let mailbox = match (&args.mailbox, &args.mailbox_capacity) {
        (Some(_), Some((_, capacity))) => capacity.expand_mailbox(&actor),
        (Some(_), None) => {
            MailboxExpansion::bounded(&actor, quote!(#actor::__private::DEFAULT_MAILBOX_CAPACITY))
        }
        (None, _) => MailboxExpansion::absent(&actor),
    };
    ...
    let max_in_flight = match &args.max_in_flight {
        Some((_, max_in_flight)) => max_in_flight.expand_max_in_flight(&actor),
        None if args.mailbox.is_some() => MaxInFlightExpansion::fixed(
            &actor,
            quote!({ #actor::__private::DEFAULT_MAX_IN_FLIGHT }),
        ),
        None => MaxInFlightExpansion::disabled(&actor),
    };
    let MaxInFlightExpansion {
        options: max_in_flight_options,
        scheduler,
        open: make_scheduler,
    } = max_in_flight;

    let supervision = match (&args.children, &args.max_children) {
        (Some(_), Some((_, max_children))) => max_children.expand_supervision(&actor),
        (Some(_), None) => {
            SupervisionExpansion::fixed(&actor, quote!({ #actor::__private::DEFAULT_MAX_CHILDREN }))
        }
        (None, _) => SupervisionExpansion::absent(&actor),
    };
...
#[derive(Default)]
struct ActorArgs {
    mailbox: Option<Ident>,
    mailbox_capacity: Option<(Ident, LimitSpec)>,
    mailbox_dispatch_budget: Option<(Ident, Expr)>,
    max_in_flight: Option<(Ident, LimitSpec)>,
    children: Option<Ident>,
    max_children: Option<(Ident, LimitSpec)>,
}
    ...
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if name == "mailbox" {
                ...
                reject_capability_value(input, &name, "mailbox_capacity")?;
                args.mailbox = Some(name);
            } else if name == "mailbox_capacity" {
                if args.mailbox_capacity.is_some() {
                    return Err(syn::Error::new(
                        name.span(),
                        "duplicate `mailbox_capacity` option",
                    ));
                }
                let capacity = parse_required_limit(input, &name, "mailbox capacity")?;
                args.mailbox_capacity = Some((name, capacity));
            } else if name == "mailbox_dispatch_budget" {
                if args.mailbox_dispatch_budget.is_some() {
                    return Err(syn::Error::new(
                        name.span(),
                        "duplicate `mailbox_dispatch_budget` option",
                    ));
                }
                if !input.peek(Token![=]) {
                    return Err(syn::Error::new(
                        name.span(),
                        "`mailbox_dispatch_budget` requires `= <const expression>`",
                    ));
                }
                input.parse::<Token![=]>()?;
                let budget: Expr = input.parse()?;
                validate_mailbox_dispatch_budget(&budget)?;
                args.mailbox_dispatch_budget = Some((name, budget));
            } else if name == "max_in_flight" {
                if args.max_in_flight.is_some() {
                    return Err(syn::Error::new(
                        name.span(),
                        "duplicate `max_in_flight` option",
                    ));
                }
                let limit = parse_required_limit(input, &name, "handler limit")?;
                args.max_in_flight = Some((name, limit));
            } else if name == "children" {
                if args.children.is_some() {
                    return Err(syn::Error::new(name.span(), "duplicate `children` option"));
                }
                reject_capability_value(input, &name, "max_children")?;
                args.children = Some(name);
            } else if name == "max_children" {
                if args.max_children.is_some() {
                    return Err(syn::Error::new(
                        name.span(),
                        "duplicate `max_children` option",
                    ));
                }
                let limit = parse_required_limit(input, &name, "child limit")?;
                args.max_children = Some((name, limit));
            } else {
                return Err(syn::Error::new(
                    name.span(),
                    "unsupported actor option; expected `mailbox`, `mailbox_capacity`, `mailbox_dispatch_budget`, `max_in_flight`, `children`, or `max_children`",
                ));
            }
...
// Capability flags and quantity policies remain separate grammar categories.
fn reject_capability_value(
    input: ParseStream<'_>,
    capability: &Ident,
    quantity: &'static str,
) -> syn::Result<()> {
    if input.peek(Token![=]) {
        return Err(syn::Error::new(
            capability.span(),
            format!("`{capability}` accepts no value; configure `{quantity}`"),
        ));
    }
    Ok(())
}
...
fn parse_required_limit(
    ...
    name: &Ident,
    quantity: &'static str,
) -> syn::Result<LimitSpec> {
    if !input.peek(Token![=]) {
        return Err(syn::Error::new(
            name.span(),
            format!("`{name}` requires `= <limit>`"),
        ));
    }
    input.parse::<Token![=]>()?;
    LimitSpec::parse(input, quantity)
}
...
impl LimitSpec {
    fn parse(input: ParseStream<'_>, quantity: &'static str) -> syn::Result<Self> {
        ...
        if let Expr::Call(call) = capacity {
            if expression_ident(&call.func).is_some_and(|policy| policy == "dynamic") {
                ...
                validate_limit_expression(&capacity, quantity)?;
    ...
    fn expand_max_in_flight(&self, actor: &TokenStream2) -> MaxInFlightExpansion {
        match self {
            Self::Unbounded => MaxInFlightExpansion::unbounded(actor),
            Self::Fixed(limit) => {
                let limit = expand_const_argument(limit);
                MaxInFlightExpansion::fixed(actor, limit)
            }
            Self::Dynamic => MaxInFlightExpansion::dynamic(actor, None),
            Self::DynamicWithDefault(limit) => {
                let limit = expand_const_argument(limit);
                MaxInFlightExpansion::dynamic(actor, Some(limit))
            }
        }
    }
...
impl MailboxExpansion {
    ...
    fn bounded(actor: &TokenStream2, capacity: TokenStream2) -> Self {
        ...
        Self {
            options: quote!(#actor::__private::FixedMailboxCapacity),
    ...
    fn dynamic(actor: &TokenStream2, default: Option<TokenStream2>) -> Self {
        ...
        let options = if let Some(default) = default {
            quote!(#actor::__private::DynamicMailboxCapacity<#default>)
        } else {
            quote!(#actor::__private::DynamicMailboxCapacity)
        };
        Self {
            options,
            sender: quote!(#actor::__private::BoundedSender<Self>),
    ...
    fn unbounded(actor: &TokenStream2) -> Self {
        Self {
            options: quote!(#actor::__private::UnboundedMailboxCapacity),
...
struct MaxInFlightExpansion {
    options: TokenStream2,
    scheduler: TokenStream2,
    open: TokenStream2,
}

impl MaxInFlightExpansion {
    fn disabled(actor: &TokenStream2) -> Self {
        Self {
            options: quote!(#actor::__private::NoMaxInFlight),
    ...
    fn fixed(actor: &TokenStream2, limit: TokenStream2) -> Self {
        let nonzero_limit = expand_nonzero_const(&limit, "handler limit must be greater than zero");
        Self {
            options: quote!(#actor::__private::FixedMaxInFlight),
    ...
    fn dynamic(actor: &TokenStream2, default: Option<TokenStream2>) -> Self {
        let validate_default = default.as_ref().map(|default| {
            let nonzero_default =
                expand_nonzero_const(default, "handler limit must be greater than zero");
            quote!(let _ = #nonzero_default;)
        });
        let options = if let Some(default) = default {
            quote!(#actor::__private::DynamicMaxInFlight<#default>)
        } else {
            quote!(#actor::__private::DynamicMaxInFlight)
        };
        Self {
            options,
    ...
    fn unbounded(actor: &TokenStream2) -> Self {
        Self {
            options: quote!(#actor::__private::UnboundedMaxInFlight),
...
impl SupervisionExpansion {
    ...
    fn fixed(actor: &TokenStream2, capacity: TokenStream2) -> Self {
        let nonzero_capacity =
            expand_nonzero_const(&capacity, "child limit must be greater than zero");
        Self {
            options: quote!(#actor::__private::FixedMaxChildren),
    ...
    fn dynamic(actor: &TokenStream2, default: Option<TokenStream2>) -> Self {
        let validate_default = default.as_ref().map(|default| {
            let nonzero_default =
                expand_nonzero_const(default, "child limit must be greater than zero");
            quote!(let _ = #nonzero_default;)
        });
        let options = if let Some(default) = default {
            quote!(#actor::__private::DynamicMaxChildren<#default>)
        } else {
            quote!(#actor::__private::DynamicMaxChildren)
        };
        Self {
            options,
    ...
    fn unbounded(actor: &TokenStream2) -> Self {
        Self {
            options: quote!(#actor::__private::UnboundedMaxChildren),
...
fn validate_limit_expression(capacity: &Expr, quantity: &'static str) -> syn::Result<()> {
    let Expr::Lit(expression) = capacity else {
        return Ok(());
    };
    let syn::Lit::Int(capacity) = &expression.lit else {
        return Err(limit_error(capacity));
    };
...
fn validate_mailbox_dispatch_budget(budget: &Expr) -> syn::Result<()> {
    ...
    if budget.base10_parse::<usize>()? == 0 {
        return Err(syn::Error::new(
            budget.span(),
            "mailbox dispatch budget must be greater than zero",
        ));
    }
    Ok(())
}

fn limit_error(value: impl quote::ToTokens) -> syn::Error {
