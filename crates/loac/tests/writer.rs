...
use loac::{Actor, ActorScope, Cx, Handler, Message, Shutdown, Writer, actor};
...
impl Handler<Token> for Collector {
    async fn handle(message: Token, mut cx: Cx<'_, Self>) {
        cx.with(|actor, _| actor.seen.lock().unwrap().push(message.0));
    }
}
